...
using adaptive_integration::Machine;
...
public:
  IntegrationController* controller;       ///< pointer to integration controller 
  ...
  IntegrationController::TerminationCode termination_code;                                 ///< the error code from the integrator
  /// initialize the constants pi and pi2
  static void initialize();
  /// consturctor
  StandardStableDistribution(myFloat alpha,                  ///< the structural parameter of the distribution
                             myFloat beta,                       ///< the skewness parameter of the distribution
                             IntegrationController& ctl,        ///< reference to integration controller
                             int verbose                        ///< indicator for verbose output
  ) : alpha(alpha), beta_input(beta), x_input(NAN),
  x_m_zeta_input(NAN), controller(&ctl), verbose(verbose) {
    if (!initialized) initialize();
    if (fabs(alpha-1)>100*Machine::epsilon){
  ...
  /// set or reset x - zeta
  void set_x_m_zeta(myFloat x_m_zeta  ///< [in] x - zeta whether positive or negative
            );
...
public:
  ...
  friend ostream& operator<<(ostream& os,                  ///< the output stream
                             const StandardStableDistribution& gc  ///< the StableDistriubion to print
                             );
}; // StandardStableDistribution

/// generate a sample from the stable distributioin 
...
private:
  StandardStableDistribution *std_stable_dist;                ///< a pointer to an instance of StandardStableDistribution 
  static string msgs[];                     ///< error messages from the integration 
  myFloat (*f)(myFloat, StandardStableDistribution*); ///< a pointer to function which will be applied to g 
...
public:
  ...
  Integral_f_of_g(myFloat (*f)(myFloat, StandardStableDistribution*),  ///< [in]pointer to function to be evaluated 
                  StandardStableDistribution* std_stable_dist,                 ///< [in] pointer to an instance of StandardStableDistribution 
                  IntegrationController* ctl = nullptr ///< [in] pointer to the integration controller 
                 ) : std_stable_dist(std_stable_dist), f(f),
                     controller((ctl==nullptr) ? std_stable_dist->controller : ctl) {}
  
  /// return a human readable error message
  string msg() {return msgs[termination_code];};
  
  /// return a pointer to the instance of StandardStableDistribution 
  StandardStableDistribution* get_std_stable_dist() {return std_stable_dist;}
...
myFloat C_stable_tail(myFloat alpha,  ///< the structural parameter
                      bool log_flag   ///< return the log
                      );
  
/// Functor passed to toms748_solve to find x such that g(x) == value
class gSolve {
private:
  StandardStableDistribution* std_stable_dist;   ///< pointer to an instance of StandardStableDistribution 
